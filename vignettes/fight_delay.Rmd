...
plot(lamda,rmse_vec)
```
5. using λ value to predict the test set and report the RMSE of the predicted model.
```{r,include=TRUE,eval=TRUE,echo=TRUE,results='hide'}
...
This is our RMSE value, this prediction and values depends on selecting features and amount of data, but we can see the rmse values of test data is slightly higher than training data but lower than validation data this confirms our model is not overfitted
